...
sorted_regras3 <- sort(regras3, by=c("confidence", "support"), decreasing=TRUE)
inspect(sorted_regras3)
plot(sorted_regras3[1:10], method="graph")
...
1.  {Cake,Hearty & Seasonal} =\> {Coffee} 
2.  {Bread,Extra Salami or Feta} =\> {salad}
3.  {Farm House,Toast} =\> {Coffee} 
4.  {Extra Salami or Feta,Spanish Brunch} =\> {Coffee} 
5.  {Bread,Sandwich,Spanish Brunch} =\> {Coffee}
6.  {Basket} =\> {Coffee}
7.  {Salad,Toast} =\> {Coffee}  
8.  {Scandinavian,Toast} =\> {Coffee}
9.  {Farm House,Juice} =\> {Coffee}
10. {Coke,Muffin} =\> {Coffee}

Acima estao listadas as 10 primeiras regras elaboradas pelo algoritmo apriori, percebe-se que essas regras podem parecer interessantes pois todas elas possuem um suporte alto e confianca maxima, no entanto, é importante ressaltar a frequencia alta de itens como coffee e bread em todo o dataset. Contudo, as regras acima sao interessantes no ponto de vista que elas revelam perfis de consumo eventualmente diferentes. Na primeira regra pode-se notar que no lhs ha itens que sao comprados com menos frequencia, o que indica que essa regra é mais relevante que uma regra que relacione bread e coffee, que sao itens muito frequentes. A regra 2 tambem revela-se uma regra muito interessante pois seus itens distoam dos itens que sao mais comuns nas transacoes e portanto revelam um perfil mais direcionado de consumo.
